...
data_pathname= '/Users/isidoraradenkovic/Desktop/Education/B18/Lab/B18-Wearables-Laboratory-main/DATA/B18_GAIT_data';
...
options.ssd_threshold = 0.1*G;  %choose value
options.time_threshold = 0.1; %[s]
...
options.MinPeakHeight = 0.59;      %choose value
%% Run Walking Detection and Step Detection Algorithm
%choose one of the filenames from the dataset and corresponding subject
%identifier
file_index = 12; %Size of the dataset is 19
%choose a value [1, 2, ..., N], where N are the number of files
%get the corresponding filename and subject name from the fileInfo table:
filename=fileInfo.filename(file_index); 
subject=fileInfo.subject(file_index);
